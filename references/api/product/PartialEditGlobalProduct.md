---
title: PartialEditGlobalProduct
category: product
api_name: PartialEditGlobalProduct
method: PUT
...
**Category:** product
**API:** PartialEditGlobalProduct

**Method:** PUT
**HTTP Path:** https://open-api.tiktokglobalshop.com/product/202509/global_products/{global_product_id}/partial_edit
...
| --- | --- | --- | --- |
| global_product_id (path) | string | Yes | The global product ID generated by TikTok Shop. |
...
| manufacturer | object | No | (**Deprecated**: This field is deprecated and will be removed in a future API version. Use `manufacturer_ids` instead.) The product manufacturer's details. |
...
| manufacturer_ids | array<string> | No | A comma-delimited list of manufacturer IDs. Retrieve the IDs from the [Search Manufacturers API](67066a580dcee902fa03ccf9). Default: The IDs provided when the global product was created. **Note**: Applicable only for the EU market in certain categories. Use the [Get Global Category Rules API](650a056df1fd3102b91b5b8e) to check the requirements. |
...
| package_weight.value | string | No | The package weight, which must be a positive number with up to 3 decimal places. (e.g. 1.99) |
...
| skus.price.amount | string | No | The price amount. Max: 99,999,999.99 (e.g. 1.32) |
...
| data.publish_results | array<object> | No | Results of syncing the changes in the edited product to markets where it is published. |
