...
**Method:** POST
**HTTP Path:** https://open-api.tiktokglobalshop.com/logistics/202502/warehouses/{warehouse_id}
...
| --- | --- | --- | --- |
| warehouse_id (path) | string | Yes | The warehouse ID, a unique and immutable primary key, used for all warehouse logistics. |
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| warehouse | object | No | The warehouse needs to update |
...
| warehouse.external_warehouse_id | string | No | The warehouse ID from the external system. (e.g. 7000714532876273410) |
| warehouse.first_name | string | No | Warehouse contact person's first name. Note: Only applicable in the Japan market, invalid in the other markets. (e.g. 雪梅) |
| warehouse.first_name_local_script | string | No | Hiragana or Katakana first name Applicable only for the JP market. (e.g. ゆきうめ) |
| warehouse.last_name | string | No | Warehouse contact person's last name. Note: Only applicable in the Japan market, invalid in the other markets. (e.g. 林) |
